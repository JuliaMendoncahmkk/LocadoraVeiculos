...
create table Veiculo (
    ...
    estado enum('Disponivel', 'Alugado', 'Manutenção'),
...
SELECT SUM(valorTotal) FROM Pagamento WHERE estado != 'Pendente';
...
## COUNT --> Realiza uma contagem da quantidade que tem a coluna escolhida. Exemplo: COUNT(modelo) - Conta o total de linhas na coluna modelo
 
SELECT modelo, marca, COUNT(LocacaoVeiculo.IdLocacao) alugados
FROM Veiculo
INNER JOIN LocacaoVeiculo on Veiculo.IdVeiculo = LocacaoVeiculo.IdVeiculo
INNER JOIN Locacao on Locacao.IdLocacao = LocacaoVeiculo.IdLocacao
...
## Dica: Utilize a clausula group by
SELECT nome, valorTotal
FROM Cliente
INNER JOIN Locacao on Cliente.IdCliente = Locacao.IdCliente
INNER JOIN Pagamento on Pagamento.IdPagamento = Locacao.IdPagamento
